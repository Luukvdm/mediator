...
type testBehavior struct {
	...
	handleFunc     func(ctx context.Context, msg mediator.Message, next mediator.Handler) (any, error)
}
...
func (b *testBehavior) Handler(next mediator.Handler) mediator.Handler {
	return mediator.HandlerFunc(func(ctx context.Context, msg mediator.Message) (any, error) {
		...
		return b.handleFunc(ctx, msg, next)
	})
}
...
func TestChain_ResultPassesThroughBehavior(t *testing.T) {
	...
	for _, c := range cases {
		...
		req.EXPECT().Handle(ctx).Return(c.result, c.err)
